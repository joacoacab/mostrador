...
- [x] **35. Fallback genérico** -- subsumida por la tarea 41
  ...
  Cuando se escribió esta tarea, "eso es Fase 3" todavía era la premisa (ver el checkpoint de Fase 1). La ampliación del 2026-08-08 trajo esa pieza a Fase 2 como tarea 41, con panel real -- no tiene sentido implementar un fallback de solo texto+log y después reemplazarlo por la escalación completa unos commits después. Se hizo directamente la tarea 41, que cubre este criterio (el mensaje fuera de alcance sí dispara algo, y queda registrado -- en `Conversation`, no en un log suelto) y lo supera.
  Hecho cuando: un mensaje fuera de alcance dispara el fallback y queda registrado en los logs del worker. -- cumplido por la tarea 41.
...
- [ ] **39. RAG Engine (pgvector + `knowledge_chunks`)** -- 🚧 en progreso, pausada
  ...
  Proveedor de embeddings: Voyage AI, modelo `voyage-4`, 1024 dims (decisión del 2026-08-08, ver conversación -- Anthropic no tiene embeddings propios y recomienda Voyage; alternativa evaluada y descartada por ahora: self-hosted, sin ventaja real al volumen de este proyecto).
  Hecho hasta ahora: imagen de Postgres cambiada a `pgvector/pgvector:pg16` (dev y prod, mismo volumen) con la extensión habilitada; app `knowledge` completa (`KnowledgeChunk` tenant-scoped, `embeddings.py` con el cliente REST de Voyage, admin que genera el embedding al cargar contenido, `GET /api/knowledge/search/` con `CosineDistance` de pgvector, bot-only); tool `consultar_base_de_conocimiento` agregada en `agent.ts` + `searchKnowledge` en `order-core.ts`. 99 tests de backend (con Voyage mockeado) + 56 de whatsapp-agent, lint, typecheck y `ruff check` en verde.
  Bloqueada en: falta la API key real de Voyage (el usuario todavía no creó la cuenta) para la verificación en vivo -- sin eso no se puede confirmar el criterio de "hecho cuando" ni tocar `VOYAGE_API_KEY` en ningún `.env`.
  Hecho cuando: una pregunta sobre contenido estático cargado (ej. una política del local) se responde con datos reales recuperados por RAG, no alucinados ni por conocimiento general del modelo -- pendiente de verificar en vivo con la API key real.

- [x] **40. Memoria en Postgres con resumen + estado**
  ...
  Backend: app `conversations` nueva -- `Conversation` (tenant-scoped, `estado`/`resumen`/`resumido_hasta_id`) y `Message` (hijo de Conversation, sin tenant propio, mismo criterio que `OrderItem`). Bot-only: `GET/PATCH /api/conversations/mine/?customer_phone=...` (trae resumen + solo los mensajes con id > resumido_hasta_id) y `POST /api/conversations/mine/messages/`. Staff (JWT, tenant-scoped): `GET /api/conversations/` -- es el "se puede consultar el estado" del criterio de esta tarea.
  `estado` queda modelado (`activa` / `requiere_atencion`) y consultable, pero esta tarea no incluye una detección automática de "esperando confirmación de pedido": no hay una señal grounded en una tool para eso sin agregar más estructura, y una heurística de texto tendría el mismo riesgo que la alucinación de la tarea 36. El campo lo usa de verdad la tarea 41 (escalamiento).
  whatsapp-agent: `memory.ts` reescrito -- `getHistory`/`appendTurn` ahora reciben el `OrderCoreClient` en vez de pegarle a Redis. Umbral de resumen: 20 mensajes sin resumir dispara un llamado chico a Claude que pliega todos menos los últimos 6 (verbatim) en `Conversation.resumen`; el costo de ese llamado se loguea con `etapa=resumen` (tarea 37). `MEMORY_TTL_SECONDS` se sacó del `.env.example` (ya no aplica, no hay TTL).
  ...
  Verificado en vivo: conversación real de 11 turnos (22 mensajes) contra el Order Core y Claude reales -- al pasar el umbral, se generó un resumen grounded (mencionó el precio real, horario, ubicación, medios de pago y que quedó pendiente lo de envíos, todo dicho de verdad en la charla) y quedaron 6 mensajes sin resumir. Un mensaje de cierre ("dale, confirmalo") post-resumen respondió coherente, confirmando el pedido creado antes. El estado se consultó con éxito vía `GET /api/conversations/` con un JWT real de staff. Efecto colateral notado (no es un bug de esta tarea): dos de los mensajes de la charla dispararon la tool de RAG (tarea 39, todavía sin `VOYAGE_API_KEY`), que devolvió 500 y encareció esos turnos por los reintentos del loop -- el bot igual respondió bien ("no tengo esa información, te contacta un operador"), matching el comportamiento esperado de degradación agraciada; se va a resolver solo cuando la tarea 39 tenga la key real. 108 tests de backend + 57 de whatsapp-agent, lint/typecheck/ruff en verde.

- [x] **41. Escalamiento a humano completo**
  ...
  Igual criterio que la tarea 36 (nunca confirmar una acción sin haberla hecho): se agregó una tool `escalar_a_humano` en vez de dejar que el modelo solo lo diga en texto -- el system prompt ahora exige llamarla, mismo patrón "CRÍTICO" que crear/cancelar pedido. El resumen corto lo escribe el propio modelo como argumento de la tool (ya tiene todo el contexto de la conversación en la ventana) en vez de gastar un llamado aparte a Claude solo para resumir.
  Backend: `POST /api/conversations/mine/escalar/` (bot-only) marca `estado=requiere_atencion` y pliega TODOS los mensajes al resumen (a diferencia del resumen corriente de la tarea 40, que deja los últimos verbatim -- una vez escalada la sigue un humano, no hace falta contexto reciente para el bot). Nuevo endpoint de staff `POST /api/conversations/<id>/resolver/` para sacarla de la lista una vez atendida; `GET /api/conversations/?estado=...` ahora filtra.
  Frontend: página nueva `/panel/conversaciones` (polling 5s, mismo patrón que `/panel` de pedidos) -- lista las conversaciones `requiere_atencion` con su resumen y un botón "Marcar como resuelta". Link agregado a la nav del panel.
  Tarea 35 (fallback genérico) quedó subsumida por esta -- ver su entrada.
  ...
  Verificado en vivo: un mensaje real de cliente enojado ("la comida llegó fría... quiero hablar con una persona YA") disparó `escalar_a_humano` de verdad -- `Conversation.estado` pasó a `requiere_atencion` con un resumen grounded ("Cliente muy molesto: comida llegó fría y lleva 40 minutos... Exige hablar con una persona"), el cliente recibió un aviso de derivación sin corte abrupto, y la conversación apareció en `GET /api/conversations/?estado=requiere_atencion` (el mismo endpoint que consume la página del panel) con un JWT real de staff. `resolver/` la sacó de esa lista. **No verificado**: la UI renderizada en un navegador real -- no tengo una herramienta de automatización de browser disponible en este entorno; sí se verificó que `npm run build` (typecheck + export estático) pasa limpio y que la página consume exactamente los mismos endpoints ya probados por curl. Recomendado: click-through manual antes de dar el panel por definitivamente probado en UI. 112 tests de backend + 62 de whatsapp-agent, lint/typecheck/ruff en verde en los tres proyectos.
